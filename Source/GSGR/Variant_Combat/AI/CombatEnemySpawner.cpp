...
void ACombatEnemySpawner::EndPlay(EEndPlayReason::Type EndPlayReason)
{
	...
	if (ObservedPlayer.IsValid())
	{
		ObservedPlayer->OnPlayerDied.RemoveDynamic(this, &ACombatEnemySpawner::OnPlayerDied);
	}
...
void ACombatEnemySpawner::SpawnEnemy()
{
	...
	if (ObservedPlayer.IsValid() && !ObservedPlayer->IsAlive())
	{
		OnPlayerDied();
		return;
	}
...
void ACombatEnemySpawner::BindToPlayerDeath()
{
	if (ObservedPlayer.IsValid())
	{
		return;
	...
	if (ACombatCharacter* Player = Cast<ACombatCharacter>(UGameplayStatics::GetPlayerCharacter(this, 0)))
	{
		ObservedPlayer = Player;
		Player->OnPlayerDied.AddUniqueDynamic(this, &ACombatEnemySpawner::OnPlayerDied);
	}
}
...
void ACombatEnemySpawner::OnEnemyDied()
{
	...
	// Zero is the explicit endless-loop value. Do not mutate it.
	if (SpawnCount == 0)
	{
	...
	// decrease a finite spawn counter
	--SpawnCount;
...
void ACombatEnemySpawner::OnPlayerDied()
{
	bStopped = true;
	GetWorld()->GetTimerManager().ClearTimer(SpawnTimer);
}
